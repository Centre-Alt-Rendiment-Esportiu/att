...
class BallPositionSensor {
  ...
  matrixMath[] mc_l[SENSOR_COUNT];
  matrixMath[] mc_r[SENSOR_COUNT];
  ...
  void importCoeffs(String coefficientFile, boolean isRightSide) {
    ...
    float[][][] coeffs = new float[SENSOR_COUNT][2][sensorSum];

    /*float[][] coeffs1 = new float[2][sensorSum];
    float[][] coeffs2 = new float[2][sensorSum];
    float[][] coeffs3 = new float[2][sensorSum];
    float[][] coeffs4 = new float[2][sensorSum];
    float[][] coeffs5 = new float[2][sensorSum];
    float[][] coeffs6 = new float[2][sensorSum];
    float[][] coeffs7 = new float[2][sensorSum];*/
    
    for(int i = 0; i < sensorSum; i++){
      for(int s = 0; s < SENSOR_COUNT; s++){
        int curSide = int(s >= SENSOR_COUNT);
        coeffs[s][curSide][i] = float(coeffs[sensorSum * s + i]);
      }
    }
    
    /*for (int i = 0; i < 28; i++) {
      coeffs1[0][i] = float(coeffs[i]);
      coeffs2[0][i] = float(coeffs[sensorSum * 1 + i]);
      coeffs3[0][i] = float(coeffs[sensorSum * 2 + i]);
      coeffs4[0][i] = float(coeffs[sensorSum * 3 + i]);
      coeffs5[0][i] = float(coeffs[sensorSum * 4 + i]);
      coeffs6[0][i] = float(coeffs[sensorSum * 5 + i]);
      coeffs7[0][i] = float(coeffs[sensorSum * 6 + i]);
      
      coeffs1[1][i] = float(coeffs[sensorSum * 7 + i]); 
      coeffs2[1][i] = float(coeffs[sensorSum * 8 + i]);
      coeffs3[1][i] = float(coeffs[sensorSum * 9 + i]);
      coeffs4[1][i] = float(coeffs[sensorSum * 10 + i]);
      coeffs5[1][i] = float(coeffs[sensorSum * 11 + i]);
      coeffs6[1][i] = float(coeffs[sensorSum * 12 + i]);
      coeffs7[1][i] = float(coeffs[sensorSum * 13 + i]);
    }*/
    
    if (isRightSide) {
      for(int i = 0; i < SENSOR_COUNT; i++){
        mc_r[i] = new matrixMath(coeffs[i]);
      }
    } else{
      for(int i = 0; i < SENSOR_COUNT; i++){
        mc_l[i] = new matrixMath(coeffs[i]);
      }
    }
