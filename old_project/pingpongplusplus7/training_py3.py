...
BAUD = 115200
PORT = 8    # PORT = 5 means COM6. FOR WINDOWS
...
def parse_hit(line):
    print(line);
    hit_re = re.match(b'hit: {(?P<sensor_one>\d+) (?P<sensor_two>\d+) (?P<sensor_three>\d+) (?P<sensor_four>\d+) (?P<side>[lr])}', line)
    if hit_re is None:
        print("none");
        return None
    else:
    ...
    for point in range(len(positions)): 
        while(True):
            ...
            if (is_OK):
                ...
                if not DEBUG:
                    training_hits.write(''.join(str(i) for i in hit_strings)) 
...
def prompt_if_OK():
    while(True):
        user_response = input("Were all the tests OK? Press enter if OK, or enter 'r' to redo: ")
        if user_response == 'r':
            return False
        elif user_response == '':
            return True
...
if __name__ == '__main__':
    ...
    side = None
    while side is None:
        side = input("Calibrate [l]eft side only, [r]ight side only, or [b]oth?: ")
